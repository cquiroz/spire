...
object Checked {
   ...
   * will be thrown.
   */
  // def checked[A](n: A): A = n //macro checkedImpl[A]
  inline def checked[A](inline n: A): A = ${ checkedImpl('{n}) }
  ...
  private def checkedImpl[A](n: Expr[A])(using Quotes, Type[A]): Expr[A] =
    ...
    tree match
      ...
      case r @ Inlined(_, _, Select(x, "unary_-")) =>
        x.asExprOf[A] match
          case '{ $a: Int } =>
            '{ if ($a == Int.MinValue) throw new spire.macros.ArithmeticOverflowException() else ${r.asExprOf[A]} }
          case '{ $a: Long } =>
