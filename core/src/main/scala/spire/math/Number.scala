...
trait NumberInstances {
  implicit final val NumberAlgebra: Field.WithDefaultGCD[Number] with CRing[Number] with NRoot[Number] with Trig[Number] with IsRational[Number] with TruncatedDivisionCRing[Number] with Signed[Number] with Order[Number] = new NumberAlgebra
}
