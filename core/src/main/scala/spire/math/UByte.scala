...
trait UByteInstances {
  implicit final val UByteAlgebra
    : CRig[Byte] with IsIntegral[UByte] with TruncatedDivision[UByte] with SignedAdditiveCMonoid[UByte] =
    new UByteAlgebra
  implicit final val UByteBitString: BitString[Byte] = new UByteBitString
